...
- (void) insertItemAtIndex: (NSInteger) index animated: (BOOL) animated {
    ...
    if (animated) {
       ...
       [UIView animateWithDuration: INSERT_DURATION delay: 0.0 options: UIViewAnimationOptionCurveEaseOut
        animations: ^{
            for (NSNumber *number in _itemViews) {
                NSInteger index = [number integerValue];
                UIView *view = [_itemViews objectForKey: number];
                if (view != itemView)  [self transformItemView: view atIndex: index];
        ...
        [CATransaction commit];
         ...
         //[self performSelector:@selector(fadeInItemView:) withObject:itemView afterDelay: 5.0];
    }
    else {
        [self disableAnimation];
        [self transformItemViews];
