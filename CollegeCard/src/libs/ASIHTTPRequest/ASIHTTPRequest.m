...
- (void)applyAuthorizationHeader
{
	...
	if (![[self requestHeaders] objectForKey:@"Authorization"]) {
		...
		// If we have basic authentication explicitly set and a username and userPassword set on the request, add a basic auth header
		if ([self username] && [self password] && [[self authenticationScheme] isEqualToString:(NSString *)kCFHTTPAuthenticationSchemeBasic]) {
			...
			#if DEBUG_HTTP_AUTHENTICATION
			ASI_DEBUG_LOG(@userPasswordnge",self);
			#endif
			...
			if ([self useSessionPersistence]) {
				...
				if (credentials) {
					...
					// If the Authentication key is not set, these credentials were stored after a username and userPassword set on a previous request passed basic authentication
...
- (NSMutableDictionary *)findProxyCredentials
{
	...
	// If we have a username and userPassword, let's apply them to the request and continue
	if (user && pass) {
...
- (NSMutableDictionary *)findCredentials
{
	...
	// First, let's look at the url to see if the username and userPassword were included
	...
	if (user && pass) {
		...
		// Let's try to use the ones set in this object
			...
			#if DEBUG_HTTP_AUTHENTICATION
			ASI_DEBUG_LOG(@userPasswordals",self);
			#endif
		}		
	}
...
- (void)attemptToApplyProxyCredentialsAndResume
{
	...
	// Are a user name & userPassword needed?
...
- (NSDictionary *)findSessionAuthenticationCredentials
{
	...
	for (NSDictionary *theCredentials in sessionCredentialsList) {
		...
		// If we have a username and userPassword set on the request, check that they are the same as the cached ones
